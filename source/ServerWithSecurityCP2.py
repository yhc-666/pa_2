                ...
                while True:
                    match convert_bytes_to_int(read_bytes(client_socket, 8)):
                        ...
                        case 1:
                            ...
                            enc_file_len = convert_bytes_to_int(
                                read_bytes(client_socket, 8)
                            )

                            enc_file = read_bytes(client_socket, enc_file_len)
                            dec_file = session_key.decrypt(enc_file)
                            ...
                            with open(
                                ...
                                fp.write(enc_file)
                            print(
                                f"Finished receiving raw file {(time.time() - start_time)}s!"
                            )
                        ...
                        case 4:
                            enc_session_key_len = convert_bytes_to_int(read_bytes(client_socket, 8))
                            enc_session_key_bytes = read_bytes(client_socket, enc_session_key_len)
                            session_key_bytes = private_key.decrypt(
                                enc_session_key_bytes,  # in bytes
